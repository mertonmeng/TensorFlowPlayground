...
#output_label = np.zeros((raw_data.shape[0], 2))

input_data = raw_data[:, 2:4].astype(np.float32)
output_label = raw_data[:, 0:2].astype(np.float32)
...
with tf.Session(graph=graph) as session:
  ...
  weight_arr = weights.eval()
  biases_arr = biases.eval()
  inv_weight = np.linalg.inv(weight_arr)
  test_est = np.matmul(test_label - biases_arr, inv_weight)
  test_data_raw = test_data*std + mean
  test_est = test_est * std + mean
  a = 1